        ...
        static private void LogOutput(LogLevel level, string strFileLine, string strFormat, params object[] args)
        {
            ...
            switch (level)
            {
                case LogLevel.LogLevel_Info:
                    strLog = string.Format("{0}{1} (at {2}) - {3}", "[Info]", strOutLog, strFileLine, strTime);
                    UnityEngine.Debug.Log(strLog, null);
                    break;
                case LogLevel.LogLevel_Trace:
                    strLog = string.Format("{0}{1} (at {2}) - {3}", "[Trace]", strOutLog, strFileLine, strTime);
                    UnityEngine.Debug.Log(strLog, null);
                    break;
                case LogLevel.LogLevel_Error:
                    strLog = string.Format("{0}{1} (at {2}) - {3}", "[Error]", strOutLog, strFileLine, strTime);
                    UnityEngine.Debug.LogError(strLog, null);
                    break;
                case LogLevel.LogLevel_Group:
                    strLog = string.Format("<color=green>[G:{0}]</color> {1} {2} - {3}", m_strGroupName, strOutLog, strFileLine, strTime);
                    UnityEngine.Debug.Log(strLog, null);
                    break;
                case LogLevel.LogLevel_Warning:
                    strLog = string.Format("{0}{1} (at {2}) - {3}", "[Warning]", strOutLog, strFileLine, strTime);
